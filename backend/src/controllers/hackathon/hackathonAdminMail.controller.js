...
export const adminSendTeamMail = async (req, res) => {
  const { audience = "teams", type, teamIds, mentorIds, subject, message, hackathonId } = req.body || {};
  ...
  try {
    ...
    const hId = hackathonId && Number.isInteger(Number(hackathonId)) ? Number(hackathonId) : null;
    ...
    if (audience === "teams") {
      ...
      if (type === "all") {
        ...
        const subWhere = { status: "approved" };
        if (hId) subWhere.hackathonId = hId;
        ...
        if (!approvedTeamIds.length) {
          ...
          teams = await HackathonTeam.findAll({
            where: { id: approvedTeamIds, ...teamWhere },
        ...
        teams = await HackathonTeam.findAll({ where: { id: ids, ...teamWhere } });
      }
